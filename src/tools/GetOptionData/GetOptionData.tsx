...
import { usePolygon } from "../../clients/Polygon/usePolygon"

/** This function retrieves options data from the Polygon.io API*/
export type GetOptionDataToolInputNode = AirNode<{
...
/** The output structure for the function which retrieves stock data from the Polygon.io API*/
export type GetStockDataToolOutputNode = AirNode<{
    ...
    next_url?: string
}, 'GetOptionDataToolOutputNode'>



export const GetOptionDataToolExecution:FC<{
    input: GetOptionDataToolInputNode,
    returnFn: (output: Record<string, any>) => void
}> = ({input, returnFn}) => {
    const polygon = usePolygon()
    useEdge(async ([{optionsTicker, multiplier, timespan, from, to}]) => {
        returnFn(await polygon.stocks.aggregates(
            optionsTicker, 
            multiplier, 
            timespan, 
            from, 
